...
description: |
  First quantitative approbation. Data preprocess workflow
author:
...
* `edulvl1` --- a highest level of [first] education
  - `school` / `college` / `bachelor` / `srecialist` / `master` / `postgrad`
...
```{r}
colnames(taia_quality) <- c("ID", "Number of misses", "Proportion of misses", "Minimum miss", "Maximum miss")
kable(taia_quality)
```
...
```{r}
taia_quality %>% 
  filter(`Number of misses` > 5) -> excluded_subjects
```
```{r}
